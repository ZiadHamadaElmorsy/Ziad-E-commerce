...
describe('MediaService', () => {
  ...
  describe('createUpload', () => {
    ...
    it('classifies video/* as VIDEO and everything else as FILE', async () => {
      media.create.mockResolvedValue(
        createdRow({ mediaType: MediaType.VIDEO, mimeType: 'video/mp4' }),
      );
      await service.createUpload({ data: Buffer.from('x'), contentType: 'video/mp4' });
      expect(media.create).toHaveBeenCalledWith(
        tx,
        expect.objectContaining({ mediaType: MediaType.VIDEO, mimeType: 'video/mp4' }),
      );

      media.create.mockResolvedValue(
        createdRow({ mediaType: MediaType.FILE, mimeType: 'application/pdf' }),
      );
      await service.createUpload({ data: Buffer.from('x'), contentType: 'application/pdf' });
      expect(media.create).toHaveBeenCalledWith(
        tx,
        expect.objectContaining({ mediaType: MediaType.FILE, mimeType: 'application/pdf' }),
      );
    });
